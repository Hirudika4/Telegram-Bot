...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Hi! Mention me with a topic like '@Hirus_Joke_Bot python' to get a joke")

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Mention me with a topic like '@Hirus_Joke_Bot python', to get some funny jokes") 
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if f'@{bot_username}' in msg:
        ...
        if match and match.group(1).strip():
            ...
            await update.message.reply.text("Please specify a topic after mentioning me")
